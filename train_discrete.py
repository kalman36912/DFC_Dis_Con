...
DATASET = "MTFL"   # MNISTUSPS ReverseMNIST Office HAR MTFL Census Crime
CLASS_NUM = 2
SENSITIVE_NUM = 2
SENSITIVE_DIM = 1
REPRESENTATION_NUM = 2
INPUT_DIM = 2048
SENSITIVE_TYPE = "Discrete"
RANDOM_SEED = 1002
...
parser.add_argument("--is_groupwise_decoder", help="", default=True)
...
## Discrete case
##import dataset
train_loader, test_loader, class_num = generator.get_dataloader_discrete(
    dataset=args.dataset, dateset_mode=1, batch_size=args.batch_size, path=None, args=args)
dataset_all = generator.get_dataloader_discrete(dataset=args.dataset, dateset_mode=2, batch_size=args.batch_size,
                                                path=None, args=args)
print("dataset_all: ", dataset_all[0].shape)
net = UDFCdis(args).cuda()
print("Model structure: ", net)
print("*********************************Learn**************************************")
_, _, _, _, _ = net.run(train_dataloader=train_loader, test_dataloader=test_loader,
                                               dataset_all=dataset_all, visual_flag=False)
