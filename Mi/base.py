...
class ParametricInfoEstimator(nn.Module):
    ...
    def learn(self, x, y):
        ...
        # divide train & val
        n = len(x)
        x_train, y_train, x_val, y_val = utils.divide_train_val(x, y)
        ...
        # learn in loops
        optimizer = torch.optim.Adam(filter(lambda p: p.requires_grad, model.parameters()), lr=model.lr,
                                     weight_decay=model.wd)
        n_batch, n_val_batch = len(x_train) // bs if len(x_train) > bs else 1, len(x_val) // bs if len(
            x_val) > bs else 1
    ...
    def estimate(self, x, y):
        model = self
        x_train, y_train, x_val, y_val = utils.divide_train_val(x, y)
        x_chunks, y_chunks = utils.get_trunks(x_val, y_val, model.bs)
        model.eval()
